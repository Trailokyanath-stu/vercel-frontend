...
export default function ReservationModal() {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    setSubmitting(true);
    setFeedback({ text: 'Sending your request…', type: '' });

    try {
      const data = await submitReservation(form);
      setFeedback({ text: data.message || 'Reservation received!', type: 'success' });
      setForm(initialForm);
      setTimeout(() => {
        closeModal();
      ...
      <div className="modal-box" role="dialog" aria-modal="true" aria-labelledby="reservationTitle">
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div className="form-field">
            ...
            <textarea id="rNotes" name="notes" rows="3" maxLength={500} value={form.notes} onChange={handleChange} />
          </div>
